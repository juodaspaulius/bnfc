...
cf2Template :: ModuleName -> ModuleName -> ModuleName -> Bool -> Bool -> CF -> String
cf2Template skelName absName errName functor pos cf = unlines
    ...
    , "failure x = Bad" ++ (if pos then " noPos" else "") ++" $ \"Undefined case: \" ++ show x\n"
    , unlines $ map (render . \(s,xs) -> case_fun functor pos s xs) $ specialData cf ++ cf2data cf
    ]
...
case_fun :: Bool -> Bool -> Cat -> [(Fun,[Cat])] -> Doc
case_fun functor' pos cat xs = vcat
  ...
  where
    ...
    ifpos doc | pos && not (isTokenCat cat) = doc
              | otherwise = empty
    ...
    mkOne (cons, args) =
        ...
        in  text cons <+> ifpos "_" <+> iffunctor "_" <+> hsep ns <+> "-> failure x"
