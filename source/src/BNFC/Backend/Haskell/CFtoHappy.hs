...
cf2Happy name absName lexName errName opts cf
 = unlines
    [header name absName lexName errName mode byteStrings',
     ...
     render $ prRules functor' (rulesForHappy absName functor' pos' cf),
...
rulesForHappy :: String -> Bool -> Bool -> CF -> Rules
rulesForHappy absM functor pos cf = map mkOne $ ruleGroups cf
  where
    mkOne (cat,rules) = (cat, map (constructRule absM functor pos reversibles cat) rules)
...
-- ("ListA A ','","flip (:) $1 $2")
constructRule :: String -> Bool -> Bool -> [Cat] -> NonTerminal -> Rule -> (Pattern,Action)
constructRule absName functor pos revs nt r0@(Rule fun cat _) = (pattern, action)
  where
